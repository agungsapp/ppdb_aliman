						...
						<div class="card">
								<div class="card-header">
										<div class="card-title">
												edit data siswa <strong class="text-capitalize text-decoration-underline">{{ $siswa->nama }}</strong>
										</div>
								</div>
								<div class="card-body">
										<form action="{{ route('admin.peserta.update', $siswa->id) }}" method="post">
												@csrf
												@method('PUT')
												<div class="row">
														<div class="col-12">
																<x-input-text nama="nama" placeholder="masukan nama ..."
																		value="{{ $siswa->nama }}">Nama</x-input-text>
														</div>
														<div class="col-12">
																<x-input-text nama="email" placeholder="masukan email ..."
																		value="{{ $siswa->email }}">Email</x-input-text>
														</div>
														<div class="col-12">
																<x-input-text nama="username" placeholder="masukan username ..."
																		value="{{ $siswa->username }}">Username</x-input-text>
														</div>
														<div class="col-12">
